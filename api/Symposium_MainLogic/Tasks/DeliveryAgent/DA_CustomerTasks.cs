...
namespace Symposium.WebApi.MainLogic.Tasks.DeliveryAgent
{
    public class DA_CustomerTasks : IDA_CustomerTasks
    {
        ...
        public DA_CustomerTasks(IDA_CustomerDT _custDT, IDA_AddressesDT _addrDT, IDA_LoyaltyDT _loyaltyDT, LocalConfigurationHelper configHlp, LoginFailuresHelper loginFailuresHlp, ICashedLoginsHelper cashedLoginsHelper)
        {
            this.custDT = _custDT;
            this.addrDT = _addrDT;
        ...
        {
            string authTokenOld = MD5Helper.SHA1(authToken);

            //1. search the cashed list of logins
            long id = cashedLoginsHelper.LoginExists(authTokenOld);
            if (id > -1) return id;

            //2. search the DB
            long custId = custDT.LoginUser(dbInfo, authTokenOld);

            if (custId <= 0)
            {
                long idNew = cashedLoginsHelper.LoginExists(authToken);
                if (idNew > -1) return idNew;

                //2. search the DB
                long custIdNew = custDT.LoginUser(dbInfo, authToken);

                if (custIdNew <= 0)
                {
                    logger.Warn("Login Fail for Customer with OldAuthToken " + (authTokenOld ?? "<NULL>") + " And NewAuthToken " + (authToken ?? "<NULL>"));
                    throw new BusinessException(Symposium.Resources.Errors.USERLOGINFAILED);
                }
                else
                {
                    cashedLoginsHelper.AddLogin(authToken, custIdNew); //3. add login to the cashed list of logins
                    return custIdNew;
                }
            }
            else
            {
                cashedLoginsHelper.AddLogin(authTokenOld, custId); //3. add login to the cashed list of logins
                return custId;
            }
        }
        ...
        /// </summary>
        /// <param name="dbInfo"></param>
        public void MakeCustomerAnonymous(DBInfoModel dbInfo)
        {
            List<DACustomerModel> toBeAnonymousCustomers = custDT.GetToBeAnonymousCustomers(dbInfo);
            if (toBeAnonymousCustomers != null)
            {
                foreach (DACustomerModel customer in toBeAnonymousCustomers)
                {
                    List<DA_AddressModel> addresses = addrDT.getCustomerAddresses(dbInfo, customer.Id);
                    foreach (DA_AddressModel address in addresses)
                    {
                        AnonymizeAddress(address);
                        long addressId = addrDT.UpdateAddress(dbInfo, address);
                        logger.Info("Anonymized address with id: " + addressId);
                    }
                    AnonymizeCustomer(customer);
                    long customerId = custDT.UpdateCustomer(dbInfo, customer);
                    logger.Info("Anonymized customer with id: " + customerId);
                }
            }
        }

        /// <summary>
        /// Anonymizes address fields
        /// </summary>
        /// <param name="address"></param>
        private void AnonymizeAddress(DA_AddressModel address)
        {
            address.AddressStreet = "Anonymized Street";
            address.AddressNo = "***";
            address.VerticalStreet = null;
            address.Floor = null;
            address.Latitude = 0;
        ...
        /// <summary>
        /// Anonymizes customer fields
        /// </summary>
        /// <param name="customer"></param>
        private void AnonymizeCustomer(DACustomerModel customer)
        {
            customer.FirstName = null;
            customer.LastName = "Anonymized Name";
            customer.Email = null;
            customer.EmailOld = null;
            customer.Password = null;
            customer.Phone1 = null;
            customer.Phone2 = null;
            customer.Mobile = null;
            customer.Notes = null;
            customer.SecretNotes = null;
            customer.LastOrderNote = null;
            customer.VatNo = null;
            customer.Doy = null;
            customer.JobName = null;
            customer.PhoneComp = null;
            customer.Proffesion = null;
            customer.IsAnonymous = DA_CustomerAnonymousTypeEnum.IsAnonymous;
        }
        ...
        /// </summary>
        /// <param name="customer"></param>
        public void SanitizeCustomer(DACustomerModel customer)
        {
            customer.FirstName = customer.FirstName != null ? customer.FirstName.Replace("'", " ") : null;
            customer.LastName = customer.LastName != null ? customer.LastName.Replace("'", " ") : null;
            customer.Email = customer.Email != null ? customer.Email.Replace("'", " ") : null;
            customer.EmailOld = customer.EmailOld != null ? customer.EmailOld.Replace("'", " ") : null;
            customer.Password = customer.Password != null ? customer.Password.Replace("'", " ") : null;
            customer.Phone1 = customer.Phone1 != null ? customer.Phone1.Replace("'", " ") : null;
            customer.Phone2 = customer.Phone2 != null ? customer.Phone2.Replace("'", " ") : null;
            customer.Mobile = customer.Mobile != null ? customer.Mobile.Replace("'", " ") : null;
            customer.Notes = customer.Notes != null ? customer.Notes.Replace("'", " ") : null;
            customer.SecretNotes = customer.SecretNotes != null ? customer.SecretNotes.Replace("'", " ") : null;
            customer.LastOrderNote = customer.LastOrderNote != null ? customer.LastOrderNote.Replace("'", " ") : null;
            customer.VatNo = customer.VatNo != null ? customer.VatNo.Replace("'", " ") : null;
            customer.Doy = customer.Doy != null ? customer.Doy.Replace("'", " ") : null;
            customer.JobName = customer.JobName != null ? customer.JobName.Replace("'", " ") : null;
            customer.PhoneComp = customer.PhoneComp != null ? customer.PhoneComp.Replace("'", " ") : null;
            customer.Proffesion = customer.Proffesion != null ? customer.Proffesion.Replace("'", " ") : null;
            customer.SessionKey = customer.SessionKey != null ? customer.SessionKey.Replace("'", " ") : null;
            customer.AuthToken = customer.AuthToken != null ? customer.AuthToken.Replace("'", " ") : null;
            customer.Metadata = customer.Metadata != null ? customer.Metadata.Replace("'", " ") : null;
            customer.ExtId1 = customer.ExtId1 != null ? customer.ExtId1.Replace("'", " ") : null;
            customer.ExtId2 = customer.ExtId2 != null ? customer.ExtId2.Replace("'", " ") : null;
            customer.ExtId3 = customer.ExtId3 != null ? customer.ExtId3.Replace("'", " ") : null;
            customer.ExtId4 = customer.ExtId4 != null ? customer.ExtId4.Replace("'", " ") : null;
        }
