...
namespace Symposium.WebApi.MainLogic.Tasks.DeliveryAgent
{
    public class DA_CustomerMessagesTasks : IDA_CustomerMessagesTasks
    {
        ...
        {
            return MessagesDT.GetAllCustomerMessages(DBInfo);
        }
        ...
        {
            long customerMessageId = MessagesDT.UpdateDaCustomerMessage(DBInfo, Model);
            SendEmailsForCustomerMessage(DBInfo, Model);
            return customerMessageId;
        }
        ...
        /// <summary>
        /// Create customer message on new call (to be implemented)
        /// </summary>
        /// <param name="DBInfo"></param>
        /// <param name="CustomerId"></param>
        public void CreateMessageOnNewCall(DBInfoModel DBInfo, long CustomerId)
        {
            List<DA_MessagesDetailsModel> messageDetailsForCall = MessagesDT.GetOnCallMessageDetails(DBInfo);
            if (messageDetailsForCall.Count != 0)
            {
                foreach (DA_MessagesDetailsModel messageDetail in messageDetailsForCall)
                {
                    ...
                    DA_CustomerMessagesModel customerMessage = CreateCustomerMessage(mainMessage, message, messageDetail, null, null, CustomerId, null, null);
            ...
            List<DA_MessagesModel> messagesForCall = MessagesDT.GetOnCallMessages(DBInfo);
            if (messagesForCall.Count != 0)
            {
                foreach (DA_MessagesModel message in messagesForCall)
                {
                    DA_MainMessagesModel mainMessage = MessagesDT.GetMainMessageById(DBInfo, message.MainDAMessagesID);
                    DA_CustomerMessagesModel customerMessage = CreateCustomerMessage(mainMessage, message, null, null, null, CustomerId, null, null);
                    long customerMessageId = MessagesDT.InsertDaCustomerMessage(DBInfo, customerMessage);
                    SendEmailsForCustomerMessage(DBInfo, customerMessage);
                }
            }
        ...
        public void CreateMessageOnOrderCreate(DBInfoModel DBInfo, long OrderId, long CustomerId, long StaffId, long StoreId)
        {
            ...
            if (messageDetailsForCreate.Count != 0)
            {
                foreach(DA_MessagesDetailsModel messageDetail in messageDetailsForCreate)
                {
                    DA_MessagesModel message = MessagesDT.GetMessageById(DBInfo, messageDetail.HeaderId);
                    DA_MainMessagesModel mainMessage = MessagesDT.GetMainMessageById(DBInfo, message.MainDAMessagesID);
                    DA_CustomerMessagesModel customerMessage = CreateCustomerMessage(mainMessage, message, messageDetail, null, OrderId, CustomerId, StaffId, StoreId);
                    long customerMessageId = MessagesDT.InsertDaCustomerMessage(DBInfo, customerMessage);
                    SendEmailsForCustomerMessage(DBInfo, customerMessage);
                }
            }
            ...
            if (messagesForCreate.Count != 0)
            {
                ...
                {
                    DA_MainMessagesModel mainMessage = MessagesDT.GetMainMessageById(DBInfo, message.MainDAMessagesID);
                    DA_CustomerMessagesModel customerMessage = CreateCustomerMessage(mainMessage, message, null, null, OrderId, CustomerId, StaffId, StoreId);
                    long customerMessageId = MessagesDT.InsertDaCustomerMessage(DBInfo, customerMessage);
                    SendEmailsForCustomerMessage(DBInfo, customerMessage);
                }
            }
        ...
        public void CreateMessageOnOrderUpdate(DBInfoModel DBInfo, long OrderId, long CustomerId, long StaffId, long StoreId)
        {
        ...
        private DA_CustomerMessagesModel CreateCustomerMessage(DA_MainMessagesModel mainMessage, DA_MessagesModel message, DA_MessagesDetailsModel messageDetail, string messageText, long? OrderId, long CustomerId, long? StaffId, long? StoreId)
        {
            ...
            customerMessage.IsTemporary = false;
        ...
        private void SendEmailsForCustomerMessage(DBInfoModel DBInfo, DA_CustomerMessagesModel customerMessage)
        {
            ...
            DA_MainMessagesModel mainMessage = MessagesDT.GetMainMessageById(DBInfo, customerMessage.MainDAMessageID ?? 0);
            ...
            DA_MessagesModel message = MessagesDT.GetMessageById(DBInfo, customerMessage.MessageId ?? 0);
