...
namespace Symposium.WebApi.MainLogic.Tasks.DeliveryAgent
{
    public class DA_OrdersTasks : IDA_OrdersTasks
    {
        ...
        public DA_OrdersTasks(IDA_OrdersDT _ordersDT, 
            ...
            IProductDT _productDT,
            IIngredientsDT _ingredientDT,
            ...
            LocalConfigurationHelper configHlp)
        {
            ...
            this.productDT = _productDT;
            this.ingredientDT = _ingredientDT;
        ...
        public void OrderValidations(DBInfoModel dbInfo, DA_OrderModel Model)
        {
            ...
            if(Model.AccountType!=1 && Model.AccountType != 4 && Model.AccountType != 6 && Model.AccountType != 10)
                throw new BusinessException("AccountType only accepts values: cash=1, credit/debit card=4, voucher=6, prepaid cards=10");
        ...
        {
            if (Model.HasDifferentStore)
                logger.Warn($"Order has different original store and selected store. StoreId: {Model.StoreId}, StoreCode: {Model.StoreCode ?? "<null>"}, ShippingAddressId: {Model.ShippingAddressId ?? 0}");
            ...
            if (Model.ShippingAddressId > 0 && Model.ShippingAddressId!=null)
            {
                ...
                if (polygon.StoreId != Model.StoreId) //Wrong StoreId from client
                {
                    if (Model.OrderType == OrderTypeStatus.Delivery)//delivery
                    {
                        logger.Warn($" Client send Wrong StoreId. Store Id from client: {Model.StoreId}, Calculated Store Id: {polygon.StoreId}.");
                        Model.StoreId = polygon.StoreId ?? 0;
        ...
        {
            bool originalProductName = MainConfigurationHelper.GetSubConfiguration(MainConfigurationHelper.apiDeliveryConfiguration, "DA_OriginalProductName");
            foreach (var item in Model.Details)
            {
                ...
                if (item.ProductId == 0 && !String.IsNullOrWhiteSpace(item.ProductCode))
                {
                    ...
                    if(originalProductName == true)
                    {
                        item.Description = productDb.Description;
                    }
                    foreach (var extras in item.Extras)
                    {
                        ...
                        string extraDescr = productDb.Extras.Where(x => x.Code == extras.ExtrasCode).Select(x => x.Description).FirstOrDefault();
        ...
        public bool CheckDA_OpeningHours(DBInfoModel dbInfo, long StoreId,DateTime checkDate)
        {
            return ordersDT.CheckDA_OpeningHours(dbInfo,StoreId, checkDate);
        }
        ...
        {
            return ordersDT.GetOrdersForGoodysOmnirest(dbInfo, out error);
        }
        ...
        public void FillExtrasCodes(DBInfoModel dbInfo, List<DA_OrderDetails> details)
        {
            if (details != null)
            {
                foreach (DA_OrderDetails detail in details)
                {
                    if (detail.Extras != null)
                    {
                        foreach (DA_OrderDetailsExtrasModel extra in detail.Extras)
                        {
                            if (extra.ExtrasCode == null)
                            {
                                IngredientsModel ingredient = ingredientDT.GetModelById(dbInfo, extra.ExtrasId);
                                if (ingredient != null)
                                {
                                    extra.ExtrasCode = ingredient.Code;
                                }
                            }
                        }
                    }
        ...
        /// </summary>
        /// <param name="order"></param>
        public void SanitizeOrder(DA_OrderModel order)
        {
            order.AgentNo = order.AgentNo != null ? order.AgentNo.Replace("'", " ") : null;
            order.StoreCode = order.StoreCode != null ? order.StoreCode.Replace("'", " ") : null;
            order.DiscountRemark = order.DiscountRemark != null ? order.DiscountRemark.Replace("'", " ") : null;
            order.Remarks = order.Remarks != null ? order.Remarks.Replace("'", " ") : null;
            order.LastOrderNote = order.LastOrderNote != null ? order.LastOrderNote.Replace("'", " ") : null;
            order.TelephoneNumber = order.TelephoneNumber != null ? order.TelephoneNumber.Replace("'", " ") : null;
            order.LoyaltyCode = order.LoyaltyCode != null ? order.LoyaltyCode.Replace("'", " ") : null;
            order.PaymentId = order.PaymentId != null ? order.PaymentId.Replace("'", " ") : null;
            order.Metadata = order.Metadata != null ? order.Metadata.Replace("'", " ") : null;
            order.ExtObj = order.ExtObj != null ? order.ExtObj.Replace("'", " ") : null;
            order.ExtData = order.ExtData != null ? order.ExtData.Replace("'", " ") : null;
            order.ExtId1 = order.ExtId1 != null ? order.ExtId1.Replace("'", " ") : null;
            order.ExtId2 = order.ExtId2 != null ? order.ExtId2.Replace("'", " ") : null;
            order.ErrorMessage = order.ErrorMessage != null ? order.ErrorMessage.Replace("'", " ") : null;
            order.LogicErrors = order.LogicErrors != null ? order.LogicErrors.Replace("'", " ") : null;
            if (order.Details != null)
            {
                foreach (DA_OrderDetails orderDetail in order.Details)
                {
                    orderDetail.Description = orderDetail.Description != null ? orderDetail.Description.Replace("'", " ") : null;
                    orderDetail.ProductCode = orderDetail.ProductCode != null ? orderDetail.ProductCode.Replace("'", " ") : null;
                    orderDetail.ProductCategoryCode = orderDetail.ProductCategoryCode != null ? orderDetail.ProductCategoryCode.Replace("'", " ") : null;
                    orderDetail.PriceListCode = orderDetail.PriceListCode != null ? orderDetail.PriceListCode.Replace("'", " ") : null;
                    orderDetail.ItemRemark = orderDetail.ItemRemark != null ? orderDetail.ItemRemark.Replace("'", " ") : null;
                    if (orderDetail.Extras != null)
                    {
                        foreach (DA_OrderDetailsExtrasModel orderDetailExtra in orderDetail.Extras)
                        {
                            orderDetailExtra.Description = orderDetailExtra.Description != null ? orderDetailExtra.Description.Replace("'", " ") : null;
                            orderDetailExtra.ExtrasCode = orderDetailExtra.ExtrasCode != null ? orderDetailExtra.ExtrasCode.Replace("'", " ") : null;
                            orderDetailExtra.ExtraCode = orderDetailExtra.ExtraCode != null ? orderDetailExtra.ExtraCode.Replace("'", " ") : null;
                            orderDetailExtra.ProductCategoryCode = orderDetailExtra.ProductCategoryCode != null ? orderDetailExtra.ProductCategoryCode.Replace("'", " ") : null;
                            orderDetailExtra.PriceListCode = orderDetailExtra.PriceListCode != null ? orderDetailExtra.PriceListCode.Replace("'", " ") : null;
                        }
                    }
                }
            }
            if (order.AnonymousCustomer != null)
            {
                customerTask.SanitizeCustomer(order.AnonymousCustomer);
            }
        }
