...
{
    public class SalesTypeTasks : ISalesTypeTasks
    {
        ISalesTypeDT dt;

        public SalesTypeTasks(ISalesTypeDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<SalesTypeSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<SalesTypeSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
