...
{
    public class TaxTasks : ITaxTasks
    {
        ITaxDT dt;
        public TaxTasks(ITaxDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<TaxSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<TaxSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
