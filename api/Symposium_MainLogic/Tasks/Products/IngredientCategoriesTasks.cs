...
{
    public class IngredientCategoriesTasks : IIngredientCategoriesTasks
    {
        IIngredientCategoriesDT dt;

        public IngredientCategoriesTasks(IIngredientCategoriesDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<IngredCategoriesSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<IngredCategoriesSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
        ...
        {
            return dt.GetAllIngredientCategories(Store);
        }
    }
}
