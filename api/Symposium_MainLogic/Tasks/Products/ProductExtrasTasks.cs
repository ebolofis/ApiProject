...
{
    public class ProductExtrasTasks : IProductExtrasTasks
    {
        IProductExtrasDT dt;

        public ProductExtrasTasks(IProductExtrasDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<ProductExtrasSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<ProductExtrasSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
