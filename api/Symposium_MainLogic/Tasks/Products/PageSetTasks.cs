...
{
    public class PageSetTasks : IPageSetTasks
    {
        IPageSetDT dt;

        public PageSetTasks(IPageSetDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<PageSetSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<PageSetSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
