...
{
    public class PriceList_EffectiveHoursTasks : IPriceList_EffectiveHoursTasks
    {
        IPriceList_EffectiveHoursDT dt;

        public PriceList_EffectiveHoursTasks(IPriceList_EffectiveHoursDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<PriceList_EffHoursSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<PriceList_EffHoursSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
