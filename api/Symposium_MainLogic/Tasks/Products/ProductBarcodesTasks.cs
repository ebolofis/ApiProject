...
{
    public class ProductBarcodesTasks : IProductBarcodesTasks
    {
        IProductBarcodesDT dt;

        public ProductBarcodesTasks(IProductBarcodesDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<ProductBarcodesSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<ProductBarcodesSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
