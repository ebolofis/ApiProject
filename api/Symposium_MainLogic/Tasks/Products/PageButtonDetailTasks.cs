...
{
    public class PageButtonDetailTasks : IPageButtonDetailTasks
    {
        IPageButtonDetailDT dt;

        public PageButtonDetailTasks(IPageButtonDetailDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<PageButtonDetSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<PageButtonDetSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
