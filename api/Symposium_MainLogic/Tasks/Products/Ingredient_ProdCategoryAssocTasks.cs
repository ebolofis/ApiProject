...
{
    public class Ingredient_ProdCategoryAssocTasks : IIngredient_ProdCategoryAssocTasks
    {
        IIngredient_ProdCategoryAssocDT dt;

        public Ingredient_ProdCategoryAssocTasks(IIngredient_ProdCategoryAssocDT dt)
        {
            this.dt = dt;
        }
        ...
        {
            List<IngedProdCategAssocSched_Model> Upserts = model.Where(w => w.Action != 1).Select(s => s).ToList();
            List<IngedProdCategAssocSched_Model> Deleted = model.Where(w => w.Action == 1).Select(s => s).ToList();
