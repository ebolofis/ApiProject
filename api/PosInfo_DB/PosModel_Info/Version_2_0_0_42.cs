...
namespace Symposium_DTOs.PosModel_Info
{
    ...
    [DisplayName("Ver : 2.0.0.42")]
    public class Version_2_0_0_42
    {
        public List<string> Ver_2_0_0_42 { get; }

        public Version_2_0_0_42()
        {
            Ver_2_0_0_42 = new List<string>();
            Ver_2_0_0_42.Add("IF NOT EXISTS (SELECT 1 FROM sys.[views] AS v WHERE v.name = 'CoverWithMeals') \n"
                           + "	EXEC('CREATE VIEW dbo.CoverWithMeals AS  \n"
                           + "		SELECT SUM(fin.CoverBreakfast) CoverBreakfast, SUM(fin.CoverNoon) CoverNoon, SUM(fin.CoverNight) CoverNight, SUM(fin.ConsumedMeals) ConsumedMeals, \n"
                           + "			   fin.FODay, fin.Description, fin.PosDescription  \n"
                           + "		FROM ( \n"
                           + "			SELECT sum(i1.CoverBreakfast) as CoverBreakfast, sum(i1.CoverNoon) as CoverNoon, Sum(i1.CoverNight) as CoverNight, ISNULL(meals.ConsumedMeals,0) ConsumedMeals, \n"
                           + "				eod.FODay,d.Description, pi.Description PosDescription \n"
                           + "			FROM invoices i \n"
                           + "			LEFT OUTER JOIN ( \n"
                           + "				SELECT  CASE WHEN (datepart(hour,i1.Day) ) >3 and datepart(hour,i1.Day) <11 THEN isnull((i1.cover),0)  ELSE 0 END  as CoverBreakfast, \n"
                           + "					CASE WHEN (datepart(hour,i1.Day) ) >=11 and datepart(hour,i1.Day) <18 THEN isnull((i1.cover),0)  ELSE 0 END  as CoverNoon,  \n"
                           + "					CASE WHEN ( (datepart(hour,i1.Day) ) >=18 or datepart(hour,i1.Day) <=3) THEN isnull((i1.cover),0)  ELSE 0 END  as CoverNight, i1.id \n"
                           + "				FROM invoices i1 \n"
                           + "			) i1 ON i1.id=i.id \n"
                           + "			LEFT OUTER JOIN posinfo pi ON pi.id=i.PosInfoId \n"
                           + "			LEFT OUTER JOIN Department d ON d.id=pi.DepartmentId \n"
                           + "			INNER JOIN endofday eod ON eod.id=i.EndOfDayId \n"
                           + "			OUTER APPLY ( \n"
                           + "				SELECT mc.EndOfDayId, SUM(mc.ConsumedMeals) ConsumedMeals \n"
                           + "				FROM MealConsumption mc \n"
                           + "				WHERE mc.EndOfDayId = i.EndOfDayId AND mc.PosInfoId = pi.Id \n"
                           + "				GROUP BY mc.EndOfDayId \n"
                           + "			) meals \n"
                           + "			WHERE i.EndOfDayId IS NOT NULL AND eod.FODay IS NOT NULL  \n"
                           + "			GROUP BY d.Description , eod.FODay, ISNULL(meals.ConsumedMeals,0), pi.Description  \n"
                           + "		) fin \n"
                           + "		GROUP BY fin.FODay, fin.Description, fin.PosDescription') ");
            Ver_2_0_0_42.Add("ALTER VIEW [dbo].[VIEW_RPT01_InvoiceHeader_Report] AS    \n"
                           ...
                           + "       OUTER APPLY (      \n"
                           + "              SELECT SUM(odi.Discount) Discount, SUM(odi.Net) Net, SUM(odi.Total) Total      \n"
                           + "              FROM OrderDetailInvoices odi      \n"
                           + "              WHERE odi.InvoicesId = i.Id     \n"
                           + "       ) odi     \n"
                           + "       OUTER APPLY (     \n"
