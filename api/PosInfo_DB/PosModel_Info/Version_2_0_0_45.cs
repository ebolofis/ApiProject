...
{
    [DisplayName("Ver : 2.0.0.45")]
    public class Version_2_0_0_45
    {
        public List<string> Ver_2_0_0_45 { get; }

        public Version_2_0_0_45()
        {
            Ver_2_0_0_45 = new List<string>();
            Ver_2_0_0_45.Add("IF EXISTS (SELECT 1 FROM sys.tables AS t WHERE t.name = 'OrdersAndCoversPerKindAndWaiter') \n"
                           + "	EXEC('ALTER VIEW SimplyOrdersPerMonthWaiter AS  \n"
                           + "			SELECT ds.Title Title, o.WaiterCode Waiter, o.WaiterName WaiterName, o.PosDescription PosDescription, o.FO_Month FO_Month, \n"
                           + "				o.FO_Year FO_Year, SUM(o.Cover) Cover, SUM(o.Orders) Orders, SUM(o.DineIn) DineIn,  \n"
                           + "				SUM(o.Delivery) Delivery, SUM(o.TakeOut) TakeOut, SUM(o.Total) Total \n"
                           + "			FROM OrdersAndCoversPerKindAndWaiter AS o \n"
                           + "			INNER JOIN DA_Stores AS ds ON ds.Id = o.StoreId \n"
                           + "			GROUP BY ds.Title, o.WaiterCode, o.WaiterName, o.PosDescription, o.FO_Month, o.FO_Year')");
            Ver_2_0_0_45.Add("ALTER VIEW [dbo].[VIEW_RPT01_InvoiceHeader] AS   \n"
                           ...
                           + "              , CASE WHEN ISNULL(DA_Origin,0) = 3 THEN '' ELSE ISNULL(isd.CustomerRemarks,'') END CustomerComments    \n"
                           ...
                           + "              , o.OrderNotes   \n"
                           + "              , o.StoreNotes   \n"
                           + "              , CASE WHEN ISNULL(DA_Origin,0) = 3 THEN '' ELSE o.CustomerNotes END CustomerNotes \n"
                           + "              , o.CustomerSecretNotes   \n"
                           + "              , o.CustomerLastOrderNotes   \n"
                           + "              , o.DA_IsPaid   \n"
                           + "              , o.IsDelay   \n"
                           + "              , o.ItemsChanged   \n"
                           + "              , CASE WHEN ISNULL(DA_Origin,0) = 3 THEN '' ELSE ISNULL(dcsa.Notes,'') END ShippingNotes  \n"
                           + "              , CASE WHEN ISNULL(DA_Origin,0) = 3 THEN '' ELSE ISNULL(dcba.Notes,'') END BillingNotes      \n"
                           ...
                           + "       ) MaxStatus");
            Ver_2_0_0_45.Add("CREATE NONCLUSTERED INDEX [IX_DA_Stores_CronSceduler] ON [dbo].[DA_Stores] \n"
                           + "( \n"
                           + "	[CronScheduler] ASC \n"
                           + ")");

            Ver_2_0_0_45.Add(@"    if not exists(select 1 from sys.indexes where name = 'IX_DA_Customers_ExtId2')
                CREATE NONCLUSTERED INDEX[IX_DA_Customers_ExtId2] ON[dbo].[DA_Customers]
                (
                   [ExtId2] ASC
                ) ON[PRIMARY]");

            Ver_2_0_0_45.Add(@"    if not exists(select 1 from sys.indexes where name = 'IX_DA_Customers_ExtId3')
                CREATE NONCLUSTERED INDEX[IX_DA_Customers_ExtId3] ON[dbo].[DA_Customers]
                (
                   [ExtId3] ASC
                ) ON[PRIMARY]");
            Ver_2_0_0_45.Add(@"    if not exists(select 1 from sys.indexes where name = 'IX_DA_Customers_ExtId4')
                CREATE NONCLUSTERED INDEX[IX_DA_Customers_ExtId4] ON[dbo].[DA_Customers]
                (
                   [ExtId4] ASC
                ) ON[PRIMARY]");
