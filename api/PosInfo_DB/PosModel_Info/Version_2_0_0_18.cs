...
{
    [DisplayName("Ver : 2.0.0.18")]
    public class Version_2_0_0_18
    {
        public List<string> Ver_2_0_0_18 { get; }

        public Version_2_0_0_18()
        {
            Ver_2_0_0_18 = new List<string>();
            Ver_2_0_0_18.Add("ALTER PROCEDURE [dbo].[TR_GetAvailableRestaurantsDatesAndTime] @TotProfiles VARCHAR(2000),  @TotRooms VARCHAR(2000), \n"
                           ...
                           + "		SELECT 1 RestrictId, COUNT(trc.ProtelId) Occupancy, DATEPART(wk, tr.ReservationDate) WeekNo, -1 RestId, NULL ActiveDate, '' roomNo, 0 Meal_Type \n"
                           ...
                           + "				WHERE r.RestrictId = 1 AND DATEPART(wk,d.dtCur) = r.WeekNo \n"
                           + "			) r1 \n"
                           + " \n"
                           + "			OUTER APPLY ( \n"
                           ...
                           + "			) r6 \n"
                           + "			WHERE  \n"
                           + "				ted.[Date] IS NULL AND ted.[Type] IS NULL  \n"
                           + "				AND (ISNULL(tc.Capacity,0)+ISNULL(toc.Capacity,0)) - (ISNULL(@Paxes,0)+ISNULL(r6.Occupancy,0)) >= 0  \n"
                           ...
                           + "				(ISNULL(tc.Capacity,0)+ISNULL(toc.Capacity,0)) - (ISNULL(@Paxes,0)+ISNULL(r6.Occupancy,0)) >= 0  \n"
                           + "				AND (ISNULL(r1.N,0) > ISNULL(r1.Occupancy,0) OR r1.RestrictId IS NULL)  \n"
                           + "				AND ISNULL(r2.Restrict2_Occupancy,1000) > @Paxes  \n"
                           + "				AND r3.RestrictId IS NULL \n"
                           + "				AND r4.RestrictId IS NULL \n"
